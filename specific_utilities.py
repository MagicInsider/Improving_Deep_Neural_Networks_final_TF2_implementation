...
import os
from utilities import join_list_to_string
...
def load_data(root_path):
    ...
    train_dataset = File(root_path + '/datasets/train_signs.h5', "r")
    train_set_x_orig = np.array(train_dataset["train_set_x"][:])  # train set features
    train_set_y_orig = np.array(train_dataset["train_set_y"][:])  # train set labels

    dev_dataset = File(root_path + '/datasets/test_signs.h5', "r")
...
def add_results_string(root_path, model_name, results, log_filename):
    """
    Procedure. Adds evaluation results to the model directory name and logfile names
    :parameters
    ...
    # composing results strings, concatenating first literal of the metric and it's value
    log_string = [log_filename, ]
    model_string = [model_name, ]
    for _, key in enumerate(results):
        log_string.append(key[:1].upper() + str(round(results[key], 3)))
        model_string.append(key[:1].upper() + str(round(results[key], 3)))

    # compiling results strings
    log_filename_with_results = '_'.join(log_string)
    model_path_with_results = '_'.join(model_string)
    ...
    os.rename(os.path.join(root_path, model_name),
              os.path.join(root_path, model_path_with_results)
              )
...
def draw_model_diagram(model, run_root_path):
    tf.keras.utils.plot_model(model,
                              to_file=run_root_path + '/dnn_run' + get_readable_date(time())+'.png',
                              show_shapes=True,
